...
const start = [ 78.1393366563122, 11.669094776753314]; // [lng, lat] for Bus 1
const end = [ 78.17239596544715, 11.26026352413981];   // [lng, lat] for Bus 1
const firebaseBusPath = "Bus102"; // Firebase path for Bus 1
// -------------------------------------
...
fetch(`https://api.openrouteservice.org/v2/directions/driving-car?api_key=${ORS_API_KEY}&start=${start[0]},${start[1]}&end=${end[0]},${end[1]}`)
  .then(response => {
  ...
  .then(data => {
    ...
    const routeLine = L.polyline(coords, { color: 'blue', weight: 6, smoothFactor: 1.5 }).addTo(map);
    map.fitBounds(routeLine.getBounds());
    busMarker.setLatLng(coords[0]);
  })
  .catch(err => {
    console.error("Failed to load route:", err);
    alert("Failed to load route from OpenRouteService.");
  });

// Firebase live GPS data listener (for this bus)
...
// ... Firebase and map initialization above ...
...
onChildAdded(gpsRef, (snapshot) => {
  ...
  if (data && data.latitude && data.longitude) {
    ...
    // Update route dynamically from current bus location to destination
    updateRoute([latlng[1], latlng[0]], end); // Note: ORS expects [lng, lat]
  }
});
